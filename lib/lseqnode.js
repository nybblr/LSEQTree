...
class LSeqNode {
    ...
    get (index) {
        ...
        const _get = (leftSum, buildingNode, queue, currentNode) => {
            ...
            let startBeginning = index-leftSum < currentNode.subCounter/2;
            let step = startBeginning ? add : subtract;
            if (!startBeginning) {
            ...
            while ((startBeginning && leftSum <= index) ||
                ...
                if (currentNode.children[i]._hasElement()) {
                    leftSum = step(leftSum, 1);
                };
                leftSum = step(leftSum,
            ...
            // #3 build path
            let newNode = new LSeqNode([currentNode.children[i].t], null);
            if (buildingNode) {
                queue.add(newNode);
            } else {
                buildingNode = newNode;
            };
            queue = newNode;
     ...
     */
    static fromJSON (o) {
        let isLeaf = o.children.length === 0;
        let node = new LSeqNode([Triple(o.t.p, o.t.s, o.t.c)], o.e);

        if (!isLeaf){
            node.children.push(LSeqNode.fromJSON(o.children[0]));
        };
