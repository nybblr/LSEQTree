...
class Identifier {
    ...
    fromNode (node) {
        ...
        for (let i = 0; i < length ; ++i) {
            ...
            // #1c copy the digit
            this._d = this._d + BigInt(node.t.p);
            if (i !== length - 1) {
                this._d = this._d << BigInt(this._base.getBitBase(i+1));
            };
            node = node.child;
        };
    ...
    toNode (e) {
        ...
        for (let i = 0; i < this._c.length; ++i) {
            ...
            mine = mine >> BigInt(dBitLength - this._base.getSumBit(i));
            // #1b copy value in the result
            resultPath.push(
                new Triple(Number(mine % BigInt(Math.pow(2, this._base.getBitBase(i)))),
                           this._s[i],
                           this._c[i]));
        };
    ...
    compareTo (o) {
        ...
        while (comparing && i < Math.min(this._c.length, o._c.length) ) {
            ...
            // #1a truncate mine
            mine = this._d;
            mine = mine >> BigInt(dBitLength - sum);
            // #1b truncate other
            other = o._d;
            other = other >> BigInt(odBitLength - sum);
            // #2 Compare triples
            // #A digit
            if (mine !== other) {
                if (mine > other) {
